...
const OrderDetailsRenderItem: React.FC<orderDetailProps> = (props) => {
    ...
    return (
        <Pressable style={{ backgroundColor: colors.commonWhite, width: WIDTH * 0.9, height: 100, alignSelf: 'center', padding: HEIGHT * 0.01, borderRadius: 12, marginBottom: HEIGHT * 0.02, shadowColor: colors.commonWhite, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 5 }}>
            {orderArray.map((details, index) => (
                <View key={index} style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: WIDTH * 0.01, marginBottom: HEIGHT * 0.01 }}>
                    <Text style={{ color: colors.brownColor, fontSize: 16, fontWeight: '600', marginRight: 10, width: WIDTH * 0.25 }}>
                        {details.name}
                    </Text>
